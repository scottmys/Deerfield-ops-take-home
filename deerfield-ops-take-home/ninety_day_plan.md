...
This plan is sequenced to hit every deadline David gave directly: the biotech pilot's three week clock, the Investment Committee review at six weeks, and budget planning at day 75. Cross pod visibility is scheduled to land before day 60 specifically because of the six week deadline, not as a stretch goal. Nothing below depends on moving faster than planned to succeed.
...
**Stand up all five coordination mechanisms immediately.** The DI Hiring Sync, DI Finance Sync, DI Compliance Review, the novel build flag at intake, and the Director of Operations owning vendor and contract coordination end to end, all detailed in the coordination model. None of these cost anything to start, they are calendar time, not build time, so they begin in week one alongside everything else here. What gets discussed inside them, the financial model, the fast lane's named owners, the templates, builds out over the following 60 days.

Three of the five front load on purpose, rather than starting at their steady state cadence. Hiring runs weekly through day 30, given the pace of hiring and the newness of the recruiter search, then steps to biweekly. Finance runs weekly for the first two weeks while the vendor inventory gets populated and validated, then steps to biweekly. Compliance runs weekly through day 30 while the fast lane criteria get calibrated against real requests and the access control matrix gets built, then steps to monthly. The novel build check in and the vendor and contract coordination workflow are event triggered rather than calendar based, so they scale with volume automatically and did not need the same adjustment.

**Immediate, week one.** Chase down the missing data sharing agreement for the biotech pilot alongside Marcus, who is already working the portfolio company's clinical ops lead. This is the one live, time boxed problem in the entire plan, with a real three week clock on it. It is a direct instance of the gap the statement of work template below is meant to prevent going forward.

**Hiring, the headline of this window.** Name a single accountable owner for offer stage momentum once a candidate reaches the offer step. Nobody currently owns pushing that stage to close, and that gap already cost a candidate to Google. Fix the handoff so every new hire is tagged to the correct cost center before their first expense report, this breaks today with no owner on the DI side.

**Coordination.** Stand up one weekly sync across both pods, aimed directly at the dependency blind spot that cost Marcus a two week promise that took six. Stand up one lightweight intake so requests to HR, Finance, and Compliance route through a single place, instead of direct email with no ticketing.
...
**Land the financial model commitment.** The vendor tracker is already live and in use since the first 30 days. What lands here is the financial model with James, in finished, usable shape, since budget planning for next year starts around day 75.
